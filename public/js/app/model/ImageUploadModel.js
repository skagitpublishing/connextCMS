define([
  ...
  //Create local Model to represent the ImageUpload model (image data) I'll retrieve from the server.
  var ImageUploadModel = Backbone.Model.extend({
    ...
    //url: 'http://'+global.serverIp+'/api/imageupload/'+this.id+'/update', 
    url: '',
    ...
    initialize: function() {
      ...
      this.url = 'http://'+global.serverIp+'/api/imageupload/'+this.id+'/update';
    },
    ...
    save: function() {
      ...
      $.getJSON(this.url, this.attributes, function(data) {
        ...
        //debugger;
        log.push('ImageUploadModel.save() executed.');
        ...
        //debugger;
        log.push('Error while trying ImageUploadModel.save(). Most likely due to communication issue with the server.');
        sendLog();
        console.error('Communication error with server while execute ImageUploadModel.save()');
      });
