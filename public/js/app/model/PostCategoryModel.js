define([
  ...
  var PostCategoryModel = Backbone.Model.extend({
    ...
    //url: 'http://'+global.serverIp+'/api/postcategory/'+this.id+'/update', 
    url: '',
    ...
    initialize: function() {
      ...
      this.url = 'http://'+global.serverIp+'/api/postcategory/'+this.id+'/update';
    },
    ...
    save: function() {
      ...
      $.getJSON(this.url, this.attributes, function(data) {
        ...
        //debugger;
        log.push('Error while trying PostCategoryModel.save(). Most likely due to communication issue with the server.');
        sendLog();
        console.error('Communication error with server while execute PostCategoryModel.save()');
      });
